...
public class readdata : MonoBehaviour
{
    ...
    public Dictionary<string, List<string>>
        jointTimestamps = new Dictionary<string, List<string>>();

    public Dictionary<string, List<int>>
        jointmaxmin = new Dictionary<string, List<int>>();

    public Dictionary<string, List<DataSnapshot>>
        jointDataTimeSnapshot = new Dictionary<string, List<DataSnapshot>>();
    ...
    Dictionary<string, string> nameToRootKeyMap = new Dictionary<string, string>();
    ...
    void Start()
    {
        ...
        {
            StartCoroutine(WaitForFirebaseAndReadData());
        }
    }
    IEnumerator WaitForFirebaseAndReadData()
    {
        // Wait until FirebaseAuthManager is ready
        yield return new WaitUntil(() => FirebaseAuthManager.Instance != null);

        // Get the current FirebaseAuthManager instance
        ...
        if (currentUser != null)
        {
            Debug.Log("User is logged in. Fetching data for user ID: " + currentUser.UserId);
            callowndata(currentUser.UserId);
        }
        else
        {
            Debug.LogError("User is not logged in or Firebase has not initialized.");
        }
    }
    void callowndata(string uid)
    {
        fb.ReadName(uid, OnReadNameComplete);
    }
    IEnumerator GetRootNodes()
    {
        var task =
            FirebaseDatabase.DefaultInstance.GetReference("/").GetValueAsync();
        yield return new WaitUntil(() => task.IsCompleted);

        if (task.Exception != null)
        {
            Debug
                .LogWarning($"Failed to fetch Firebase data: {task.Exception}");
            yield break;
        }
        ...
        foreach (var childSnapshot in snapshot.Children)
        {
            ...
            // Access the "Patient_Details" child node
            var patientDetails = childSnapshot.Child("Patient_Details");
            if (patientDetails.Exists)
            {
                // Get the "Name" field from the "Patient_Details"
                var name = patientDetails.Child("Name").Value?.ToString();
                if (!string.IsNullOrEmpty(name))
                {
                    // Add to the rootNodes list and map the name to the root key
                    rootNodes.Add(name);
                    nameToRootKeyMap[name] = rootKey;
                }
                else
                {
                    rootNodes.Add("Name not found");
                }
            }
            else
            {
                rootNodes.Add("Patient_Details not found");
            }
        }
        foreach (var rootNode in rootNodes)
        {
            userId_Dropdown
                .options
                .Add(new Dropdown.OptionData(rootNode.ToString()));
        }
    }

    public void onuseridDropdownValueChanged()
    {
        string selectedName = userId_Dropdown.options[userId_Dropdown.value].text;
        Debug.Log("Selected Name: " + selectedName);

        // Get the corresponding root key using the dictionary
        if (nameToRootKeyMap.TryGetValue(selectedName, out string rootKey))
        {
            Debug.Log("Root Key: " + rootKey);
            fb.ReadName(rootKey, OnReadNameComplete);
        }
        else
        {
            Debug.LogError("Root key not found for the selected name.");
        }
    ...
    private void OnReadNameComplete(IEnumerable<DataSnapshot> childrenList)
    {
        ...
        if (alldata != null)
        {
            ...
            foreach (var
                childSnapshot
                in
                alldata //childSnapshit (joint,Patient_Details)
            )
            {
                if (childSnapshot.Key == "Joint")
                {
                    ...
                    foreach (var
                        jointChild
                        in
                        jointype.Children // joint child: elbow,knee1,...
                    )
                    {
                        jointDataTimeSnapshot
                            .Add(jointChild.Key.ToString(),
                            new List<DataSnapshot>());
                        jointsAvailable.Add(jointChild.Key.ToString());
                        joint_Dropdown
                            .options
                            .Add(new Dropdown.OptionData(jointChild
                                    .Key
                                    .ToString()));
                        jointTimestamps
                            .Add(jointChild.Key.ToString(), new List<string>());
                        jointmaxmin
                            .Add(jointChild.Key.ToString(), new List<int>());

                        //if timestamp dictionary for graph does not havde that join make new key with empty list
                        if (
                            !jointTimestamps
                                .ContainsKey(jointChild.Key.ToString())
                        )
                        {
                            jointTimestamps
                                .Add(jointChild.Key.ToString(),
                        ...
                        foreach (var
                            timestampSnapshot
                            in
                            jointChild.Children //timestamp of joint
                        )
                        {
                            string timestamp1 = timestampSnapshot.Key;
                            jointTimestamps[jointChild.Key].Add(timestamp1);
                            jointDataTimeSnapshot[jointChild.Key]
                                .Add(timestampSnapshot);
                            DataSnapshot datachild =
                                timestampSnapshot.Child("data");
                            foreach (var child in datachild.Children)
                            {
                                foreach (var
                                    valuesMin
                                    in
                                    child.Children //for graph max min values
                                )
                                {
                                    if (valuesMin.Key == "maxmin")
                                    {
                                        string[] components =
                                            valuesMin
                                                ...
                                                .Trim('(', ')')
                                                .Split(',');
                                        jointmaxmin[jointChild.Key]
                                            .Add(int.Parse(components[0]));
                                    }
                                }
                            }
                        }
    ...
    public void OnDropdownValueChanged()
    {
        ...
        if (index >= 0 && index < childlastdata.Count)
        {
            ...
            DatabaseReference parent_timestamp =
                selectedSnapshot.Reference.Parent;
    ...
    private void DisplayCurrentIndex()
    {
        ...
        if (shotdata.Count > 0)
        {
            ...
            if (currentEntry.ContainsKey("maxmin"))
            {
                ...
                if (range != null)
                {
                    angle.text = range[1] + "to" + (range[0] - 90);
                    int init = range[0];
                    int fin = range[1];
                    ranged = true;
            ...
            else if (currentEntry.ContainsKey("pain"))
            {
                ...
                Debug
                    .Log((
                    (Dictionary<string, object>)currentEntry["pain"]
                    )["Location"]);
                Vector3 position =
                    ParseVector3((
                    string
                    )((Dictionary<string, object>)
                    currentEntry["pain"])["Location"]);
                ...
                if (Intensity != "NA")
                {
                    ...
                    if (range != null)
                    {
                        if (range.Count == 2)
                        {
                            ...
                            StartCoroutine(AcuteLegPain(range[1]-90));
                        }
    ...
    private IEnumerator MoveLegPain(int init, int fin)
    {
        ...
        {
            float t =
                Mathf.Sin(elapsed / duration * Mathf.PI * 2) * 0.5f + 0.5f;
            Debug.Log(init + " " + fin);

            // Interpolate between init and fin
            int currentPos = (int)Mathf.Lerp(-init + 90, fin, t);
