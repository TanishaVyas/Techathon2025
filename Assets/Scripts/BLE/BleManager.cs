    ...
    public class BleManager : MonoBehaviour, IBleNotify
    {
        ...
        public static BleManager Instance
        {
            get
            {
                if (_instance != null)
                {
                    if (_instance._messageAdapter == null)
                    {
        ...
        [
            Tooltip(
                "Initializes the BleManager on Unity's Awake. " +
        ...
        /// </summary>
        private readonly Dictionary<string, TaskDescription>
            _callbackNotifiers = new Dictionary<string, TaskDescription>();
...
#region Adapter
            if (_messageAdapter == null)
            {
                ...
                if (_messageAdapter == null)
                {
                    GameObject bleAdapter =
                        new GameObject(nameof(BleMessageAdapter));
                    bleAdapter.transform.SetParent (transform);

                    _messageAdapter =
                        bleAdapter.AddComponent<BleMessageAdapter>();
                }
            }
...
#region Android Library
            ...
            {
                AndroidJavaClass unityAndroidBle =
                    new AndroidJavaClass("com.velorexe.unityandroidble.UnityAndroidBLE");
                _javaBleManager =
                    unityAndroidBle
        ...
        )
        {
            _onDeviceFound = onDeviceFound;
            BleTask task =
                new BleTask("searchForBleDevicesWithFilter",
                    scanPeriod,
                    deviceMac,
                    deviceName,
                    serviceUuid);
        ...
        public void SearchForCompanionDevices(OnDeviceFound onDeviceFound)
        {
            _onDeviceFound = onDeviceFound;
            BleTask task = new BleTask("searchForCompanionBleDevices");
        ...
        )
        {
            ...
            _callbackNotifiers
                .Add(id, new TaskDescription(receiver, runsContiniously));
        ...
        private void OnBleMessageReceived(BleMessage msg)
        {
            ...
            {
                Debug
                    .LogError($"No OnBleMessage with ID {msg.ID} is in the BleManager's stack.");
                return;
            }
